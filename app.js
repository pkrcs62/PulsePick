...
function calculate(){const w=state.weights||DEFAULT_WEIGHTS;const target=state.profitTarget/state.budget*100;const arr=merge().map(s=>{const ns=newsScore(s.symbol,s.sector),volBonus=s.volume>1?1:0,trend=s.m1*w.m1W+s.m6*w.m6W+s.y1*w.y1W,raw=50+trend*w.trendW+volBonus*w.volBonusW+ns*w.newsW+(s.m1>0?2:0)+(s.m6>0?2:0)+(s.y1>0?1:0),exp=clamp((raw-50)/4+target*.35,-12,18),down=Math.max(0,7-(s.m1*.12+s.m6*.04)),positive=exp>0&&exp>=target*.55&&down<=7&&s.price<=state.budget,hm=Math.max(4,Math.round(state.days*(.35+Math.min(1,Math.max(0,exp/10))*.2))),hx=Math.max(hm+2,Math.round(state.days*(.7+Math.min(1,Math.max(0,exp/12))*.35)));return {...s,score:clamp(raw,0,100),expected:exp,down,positive,hm,hx,trend,volBonus,newsScore:ns,target:s.price*(1+Math.max(exp,target*.7)/100),stop:s.price*(1-Math.min(6,Math.max(3,down))/100)}}).filter(x=>x.positive).sort((a,b)=>b.score-a.score);state.picks=arr;let rem=state.budget,rows=[];for(const p of arr){if(rem<p.price)continue;const q=Math.min(2,Math.floor(rem/p.price));if(!q)continue;rem-=q*p.price;rows.push({...p,qty:q,invest:q*p.price,profitMin:q*p.price*p.expected*.5/100,profitMax:q*p.price*p.expected/100});if(rows.length===4)break}state.plan={rows,invest:rows.reduce((a,r)=>a+r.invest,0),cash:rem}}
...
async function connectTest(){
  ...
  $("connectionDetail").textContent="Checking Render and the Upstox connection.";
  try{
    ...
    if(!u.ok || !uj.ok) throw Error(uj.error||"Upstox market-data check failed");
    $("connectionState").textContent="Connected ✓";
    $("connectionDetail").textContent=`Upstox is connected. ${Number(uj.universeCount||0).toLocaleString("en-IN")} NSE equity instruments available; ${uj.samplePrices||0} sample prices received.`;
    $("marketStatus").textContent=`Upstox online · ${Number(uj.universeCount||0).toLocaleString("en-IN")} equities`;
    return true;
  }catch(e){
    $("connectionState").textContent="Connection failed";
    $("connectionDetail").textContent=e.message||"Unable to connect to Upstox";
    $("marketStatus").textContent="Upstox connection failed";
    return false;
  }
}
...
async function refresh(){
  setGoal();
  $("marketStatus").textContent="Scanning Indian equities with Upstox…";
  try{
    ...
    console.error(e);
    $("marketStatus").textContent="Upstox scan failed";
